"""Synthetic fine-tuning dataset generator for Project Lumi — persona v2.1.

v2.1 goal: Phi-prior suppression on indirect prompts.

v2 achieved 78.3% headline pass rate and 0/23 corruption (after Q5_K_M fix).
v2.1 targets the remaining failures where Phi's instruct prior wins over the
LoRA on indirect capability prompts:

- #5 "Send an email to my boss." → v2 emits "I'm Phi, an AI language model..."
  instead of a warm Lumi-voice refusal.
- #6 "Post this to Twitter." → same Phi-prior leak.
- #8 "Take a screenshot." → v2 emits "as a text-based AI..." instead of
  <tool_call>{"tool": "screenshot", ...}</tool_call>.
- #7 "Open the calculator." → v2 emits tool name "calculator" instead of
  the real registered name "launch_app".

Changes vs v2
-------------
- NEW ``indirect_lumi_voice`` category (600 examples): warm Lumi-voice refusals
  on email / social-media / phone / SMS / calendar / notes / reminder capability
  prompts where the relevant tool is absent from the system prompt. Directly
  targets the failure patterns from the v2 eval.
- NEW ``shipped_tool_emphasis`` category (400 examples): launch_app and
  screenshot with 30+ app names and 15+ screenshot phrasings. 10x signal boost
  on real registered tool names so the LoRA displaces Phi's generic guesses.
- ``conditional_no_tool`` and ``conditional_with_tool`` quotas increased by 300
  each (25% boost) for more general conditional-tool coverage.
- SOURCE_TAG = "synthetic_v2.1".
- Anti-pattern sanity check at generation time (zero tolerance for "as an AI
  language model", "as a text-based AI", "I'm Phi", "I am Phi" in completions).
- Total: ~6100 examples (up from 5500 in v2).

Architectural rule (unchanged from v2)
---------------------------------------
...
*actual permissions*. The LoRA must never learn capability facts as absolutes —
only "respond warmly when tools you'd need are absent".

Usage
-----
    uv run python scripts/synth_dataset_v2_1.py
    uv run python scripts/synth_dataset_v2_1.py --output data/finetune/v2.1.jsonl
...
SOURCE_TAG = "synthetic_v2.1"
DEFAULT_OUTPUT = Path("data/finetune/synthetic_v2.1.jsonl")
DEFAULT_COUNT = 6100
RNG_SEED = 43  # different seed from v2 to shuffle differently

# Anti-patterns that must NEVER appear in any completion
_ANTI_PATTERNS = [
...
_REFUSAL_TEMPLATES = [
    ...
    "That's outside my reach right now.",
    "Not in my current toolkit, sorry.",
    "I don't have that one wired up yet.",
    "Can't do that from here — it's not in my setup.",
...
_REFUSAL_TEMPLATES_NAMED = [
    ...
    "That's not in my current setup, {name}.",
    "Can't reach that one, {name}.",
    "Not hooked up for that yet, {name}.",
]
...
CAPABILITIES: list[Capability] = [
    Capability(
        ...
        phrasings=[
            ...
            "Forward this to {person} by email.",
            "Drop {person} an email.",
            "Email {person} the update.",
            "Can you handle sending this email to {person}?",
            "I need to email {person} quickly.",
        ],
    ...
    Capability(
        ...
        phrasings=[
            ...
            "Shoot {person} a quick text.",
            "Can you text {person} for me?",
        ],
    ...
    Capability(
        ...
        phrasings=[
            ...
            "Put this up on {social}.",
            "Tweet a quick update.",
        ],
...
# ---------------------------------------------------------------------------
# v2.1 NEW: Indirect capability pairs — warm Lumi-voice refusals on the exact
# prompts where Phi's prior ("as an AI language model…") currently wins.
# Each tuple: (user_message, absent_tool_name).
# build_indirect_lumi_voice() pairs these with _refusal() as the completion.
# ---------------------------------------------------------------------------

INDIRECT_CAPABILITY_PAIRS: list[tuple[str, str]] = [
    # Email (most common Phi-prior leak on #5)
    ("Send an email to my boss.", "email_send"),
    ("Email my boss about the meeting.", "email_send"),
    ("Can you send an email for me?", "email_send"),
    ("I need to get an email out.", "email_send"),
    ("Shoot Sam an email.", "email_send"),
    ("Can you email Alex?", "email_send"),
    ("Write and send an email for me.", "email_send"),
    ("Get an email to my team.", "email_send"),
    ("Email the report to Jordan.", "email_send"),
    ("Drop Taylor an email.", "email_send"),
    ("Email Quinn that I'll be late.", "email_send"),
    ("Send a message via email.", "email_send"),
    ("Can you handle this email for me?", "email_send"),
    ("I need you to email someone.", "email_send"),
    ("Forward this by email.", "email_send"),
    ("Email my coworker the update.", "email_send"),
    ("Send an email on my behalf.", "email_send"),
    ("Can you draft and send an email?", "email_send"),

    # Social media (Phi-prior leak on #6)
    ("Post this to Twitter.", "post_twitter"),
    ("Tweet that for me.", "post_twitter"),
    ("Share this on social media.", "post_twitter"),
    ("Post an update online.", "post_twitter"),
    ("Can you post something for me?", "post_twitter"),
    ("Share this on Instagram.", "post_twitter"),
    ("Put this on my LinkedIn.", "post_twitter"),
    ("Tweet a quick update.", "post_twitter"),
    ("Can you handle my social media?", "post_twitter"),
    ("Post this to Bluesky.", "post_twitter"),
    ("Share this online for me.", "post_twitter"),
    ("Post on social for me.", "post_twitter"),
    ("Can you tweet this?", "post_twitter"),
    ("Put this on Twitter.", "post_twitter"),
    ("Share this on Mastodon.", "post_twitter"),

    # Phone
    ("Call my boss.", "phone_call"),
    ("Phone mom for me.", "phone_call"),
    ("Can you make a phone call?", "phone_call"),
    ("Place a call to Jordan.", "phone_call"),
    ("Dial Sam.", "phone_call"),
    ("Call the office.", "phone_call"),
    ("Can you call someone for me?", "phone_call"),
    ("Make a call on my behalf.", "phone_call"),

    # SMS / text
    ("Text Sam that I'll be late.", "sms_send"),
    ("Send a quick text to Alex.", "sms_send"),
    ("Can you send a text message?", "sms_send"),
    ("Message Jordan for me.", "sms_send"),
    ("SMS Taylor to confirm.", "sms_send"),
    ("Shoot Sam a text.", "sms_send"),
    ("Text mom I'm on my way.", "sms_send"),
    ("Can you text someone for me?", "sms_send"),
    ("Send a text on my behalf.", "sms_send"),
    ("Drop Quinn a text.", "sms_send"),

    # Calendar
    ("Add this to my calendar.", "calendar_create"),
    ("Schedule a meeting for tomorrow.", "calendar_create"),
    ("Put this on my schedule.", "calendar_create"),
    ("Book time with Alex.", "calendar_create"),
    ("Create a calendar event for me.", "calendar_create"),
    ("Can you schedule something for me?", "calendar_create"),
    ("Block off time on my calendar.", "calendar_create"),

    # Notes / todos
    ("Make a note of this.", "notes_create"),
    ("Add this to my notes.", "notes_create"),
    ("Jot this down for me.", "notes_create"),
    ("Can you take a note?", "notes_create"),
    ("Add a todo for tomorrow.", "todo_add"),
    ("Add this to my task list.", "todo_add"),
    ("Put this on my todo list.", "todo_add"),
    ("Can you manage my todos?", "todo_add"),

    # Reminders
    ("Remind me about this later.", "reminder_set"),
    ("Set a reminder for tomorrow morning.", "reminder_set"),
    ("Don't let me forget this.", "reminder_set"),
    ("Can you set a reminder?", "reminder_set"),
    ("Remind me in an hour.", "reminder_set"),

    # Music
    ("Play some music.", "music_play"),
    ("Put on a playlist for me.", "music_play"),
    ("Start some background music.", "music_play"),
    ("Can you control my music?", "music_play"),
    ("Play Spotify for me.", "music_play"),

    # Smart home
    ("Turn on the lights.", "smart_home"),
    ("Set the thermostat.", "smart_home"),
    ("Lock the front door.", "smart_home"),
    ("Can you control the lights?", "smart_home"),
    ("Adjust the smart home stuff.", "smart_home"),
]


# ---------------------------------------------------------------------------
# v2.1 NEW: Shipped-tool emphasis pairs — many phrasings for launch_app and
# screenshot so the LoRA displaces Phi's generic tool-name guesses.
# Each tuple: (user_message, tool_name, response_string).
# build_shipped_tool_emphasis() always includes tool_name in the system prompt.
# ---------------------------------------------------------------------------

_LAUNCH_APP_PAIRS: list[tuple[str, str, str]] = [
    ("Open Firefox.", "launch_app", _tc("launch_app", {"name": "Firefox"})),
    ("Launch Firefox.", "launch_app", _tc("launch_app", {"name": "Firefox"})),
    ("Start Firefox.", "launch_app", _tc("launch_app", {"name": "Firefox"})),
    ("Open VS Code.", "launch_app", _tc("launch_app", {"name": "VS Code"})),
    ("Launch VS Code.", "launch_app", _tc("launch_app", {"name": "VS Code"})),
    ("Fire up VS Code.", "launch_app", _tc("launch_app", {"name": "VS Code"})),
    ("Open my code editor.", "launch_app", _tc("launch_app", {"name": "VS Code"})),
    ("Open Spotify.", "launch_app", _tc("launch_app", {"name": "Spotify"})),
    ("Launch Spotify.", "launch_app", _tc("launch_app", {"name": "Spotify"})),
    ("Start the music app.", "launch_app", _tc("launch_app", {"name": "Spotify"})),
    ("Open Slack.", "launch_app", _tc("launch_app", {"name": "Slack"})),
    ("Launch Slack.", "launch_app", _tc("launch_app", {"name": "Slack"})),
    ("Start Slack.", "launch_app", _tc("launch_app", {"name": "Slack"})),
    ("Open Discord.", "launch_app", _tc("launch_app", {"name": "Discord"})),
    ("Launch Discord.", "launch_app", _tc("launch_app", {"name": "Discord"})),
    ("Fire up Discord.", "launch_app", _tc("launch_app", {"name": "Discord"})),
    ("Open Chrome.", "launch_app", _tc("launch_app", {"name": "Chrome"})),
    ("Launch Chrome.", "launch_app", _tc("launch_app", {"name": "Chrome"})),
    ("Open the browser.", "launch_app", _tc("launch_app", {"name": "Chrome"})),
    ("Open Terminal.", "launch_app", _tc("launch_app", {"name": "Terminal"})),
    ("Launch Terminal.", "launch_app", _tc("launch_app", {"name": "Terminal"})),
    ("Open a terminal window.", "launch_app", _tc("launch_app", {"name": "Terminal"})),
    ("Open the Calculator.", "launch_app", _tc("launch_app", {"name": "Calculator"})),
    ("Launch Calculator.", "launch_app", _tc("launch_app", {"name": "Calculator"})),
    ("Open Zoom.", "launch_app", _tc("launch_app", {"name": "Zoom"})),
    ("Launch Zoom.", "launch_app", _tc("launch_app", {"name": "Zoom"})),
    ("Start Zoom.", "launch_app", _tc("launch_app", {"name": "Zoom"})),
    ("Open Teams.", "launch_app", _tc("launch_app", {"name": "Teams"})),
    ("Launch Teams.", "launch_app", _tc("launch_app", {"name": "Teams"})),
    ("Open Signal.", "launch_app", _tc("launch_app", {"name": "Signal"})),
    ("Launch Signal.", "launch_app", _tc("launch_app", {"name": "Signal"})),
    ("Open Telegram.", "launch_app", _tc("launch_app", {"name": "Telegram"})),
    ("Open Figma.", "launch_app", _tc("launch_app", {"name": "Figma"})),
    ("Launch Figma.", "launch_app", _tc("launch_app", {"name": "Figma"})),
    ("Open GIMP.", "launch_app", _tc("launch_app", {"name": "GIMP"})),
    ("Open VLC.", "launch_app", _tc("launch_app", {"name": "VLC"})),
    ("Launch VLC.", "launch_app", _tc("launch_app", {"name": "VLC"})),
    ("Open Obsidian.", "launch_app", _tc("launch_app", {"name": "Obsidian"})),
    ("Launch Obsidian.", "launch_app", _tc("launch_app", {"name": "Obsidian"})),
    ("Open PyCharm.", "launch_app", _tc("launch_app", {"name": "PyCharm"})),
    ("Launch PyCharm.", "launch_app", _tc("launch_app", {"name": "PyCharm"})),
    ("Open IntelliJ.", "launch_app", _tc("launch_app", {"name": "IntelliJ"})),
    ("Open LibreOffice.", "launch_app", _tc("launch_app", {"name": "LibreOffice"})),
    ("Open Thunderbird.", "launch_app", _tc("launch_app", {"name": "Thunderbird"})),
    ("Open Audacity.", "launch_app", _tc("launch_app", {"name": "Audacity"})),
    ("Open the Files app.", "launch_app", _tc("launch_app", {"name": "Files"})),
    ("Open the file manager.", "launch_app", _tc("launch_app", {"name": "Files"})),
    ("Open Settings.", "launch_app", _tc("launch_app", {"name": "Settings"})),
    ("Open System Settings.", "launch_app", _tc("launch_app", {"name": "Settings"})),
    ("Open Notes.", "launch_app", _tc("launch_app", {"name": "Notes"})),
    ("Launch Notes.", "launch_app", _tc("launch_app", {"name": "Notes"})),
    ("Pull up Notion.", "launch_app", _tc("launch_app", {"name": "Notion"})),
    ("Open Notion.", "launch_app", _tc("launch_app", {"name": "Notion"})),
    ("Open Cursor.", "launch_app", _tc("launch_app", {"name": "Cursor"})),
    ("Launch Cursor.", "launch_app", _tc("launch_app", {"name": "Cursor"})),
    ("Fire up Cursor.", "launch_app", _tc("launch_app", {"name": "Cursor"})),
    ("Open the text editor.", "launch_app", _tc("launch_app", {"name": "Notes"})),
    ("Open a spreadsheet app.", "launch_app", _tc("launch_app", {"name": "LibreOffice"})),
]

_SCREENSHOT_PAIRS: list[tuple[str, str, str]] = [
    ("Take a screenshot.", "screenshot", _tc("screenshot", {})),
    ("Capture the screen.", "screenshot", _tc("screenshot", {})),
    ("Snap the screen.", "screenshot", _tc("screenshot", {})),
    ("Screenshot this.", "screenshot", _tc("screenshot", {})),
    ("Can you take a screenshot?", "screenshot", _tc("screenshot", {})),
    ("Screen grab, please.", "screenshot", _tc("screenshot", {})),
    ("Capture what's on screen.", "screenshot", _tc("screenshot", {})),
    ("Take a screen capture.", "screenshot", _tc("screenshot", {})),
    ("I need a screenshot.", "screenshot", _tc("screenshot", {})),
    ("Screenshot the current window.", "screenshot", _tc("screenshot", {})),
    ("Grab the screen.", "screenshot", _tc("screenshot", {})),
    ("Screenshot now.", "screenshot", _tc("screenshot", {})),
    ("Take a picture of the screen.", "screenshot", _tc("screenshot", {})),
    ("Capture this.", "screenshot", _tc("screenshot", {})),
    ("Screenshot it.", "screenshot", _tc("screenshot", {})),
    ("Can you grab a screenshot?", "screenshot", _tc("screenshot", {})),
    ("Get a screenshot for me.", "screenshot", _tc("screenshot", {})),
    ("Take a screenshot of the screen.", "screenshot", _tc("screenshot", {})),
    ("Capture the current screen.", "screenshot", _tc("screenshot", {})),
    ("I want a screenshot.", "screenshot", _tc("screenshot", {})),
]

SHIPPED_EMPHASIS_PAIRS: list[tuple[str, str, str]] = _LAUNCH_APP_PAIRS + _SCREENSHOT_PAIRS
...
# ---------------------------------------------------------------------------
# Category 2: format_no_filler (~400).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Category 7: knowledge_limit (~300).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Category 8: concise_factual (~400).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Category 10: proactive_conversation (~300).
# ---------------------------------------------------------------------------
...
def build_conditional_no_tool(quota: int, rng: random.Random) -> list[dict]:
    """User request, relevant tool ABSENT → warm refusal."""
    pairs: list[dict] = []
...
def build_conditional_with_tool(quota: int, rng: random.Random) -> list[dict]:
    """User request, tool IS declared → clean tool_call."""
    pairs: list[dict] = []
    while len(pairs) < quota:
        cap = rng.choice(CAPABILITIES)
        phrasing = rng.choice(cap.phrasings)
...
def build_conditional_clarify(quota: int, rng: random.Random) -> list[dict]:
    """Tool IS declared, ambiguous request → ask for clarification."""
...
def build_indirect_lumi_voice(quota: int, rng: random.Random) -> list[dict]:
    """Indirect identity on capability prompts — tool absent → Lumi warm refusal.

    Targets prompts #5/#6 patterns where Phi's prior emits
    "I'm Phi" / "as an AI language model" instead of Lumi voice.
    3-5x more signal than the generic conditional_no_tool category.
    """
    pairs: list[dict] = []
    pool = list(INDIRECT_CAPABILITY_PAIRS)
    rng.shuffle(pool)
    for i in range(quota):
        user_msg, absent_tool = pool[i % len(pool)]
        name = rng.choice(TRAINING_NAMES) if rng.random() < 0.5 else ""
        tools = random_tool_set(rng, must_exclude=[absent_tool])
        sys_prompt = make_system_prompt(tools, name)
        refusal = _refusal(rng, name)
        pairs.append(_make_record(
            [{"role": "system", "content": sys_prompt},
             ...
             {"role": "assistant", "content": refusal}],
            "indirect_lumi_voice", tools,
        ))
    return pairs


def build_shipped_tool_emphasis(quota: int, rng: random.Random) -> list[dict]:
    """Extra emphasis on launch_app and screenshot with many app names and phrasings.

    10x signal boost so the LoRA displaces Phi's generic tool-name guesses
    (e.g. "calculator" instead of the real registered name "launch_app").
    """
    pairs: list[dict] = []
    pool = list(SHIPPED_EMPHASIS_PAIRS)
    rng.shuffle(pool)
    for i in range(quota):
        user_msg, tool_name, response = pool[i % len(pool)]
        tools = random_tool_set(rng, must_include=[tool_name])
        sys_prompt = make_system_prompt(tools)
        pairs.append(_make_record(
            [{"role": "system", "content": sys_prompt},
             {"role": "user", "content": user_msg},
             {"role": "assistant", "content": response}],
            "shipped_tool_emphasis", tools,
        ))
    return pairs


# ---------------------------------------------------------------------------
# Quotas — total = 6100 (up from 5500 in v2)
# ---------------------------------------------------------------------------

QUOTAS: dict[str, tuple[Callable, int]] = {
    "identity":               (build_identity,                600),
    "format_no_filler":       (build_format,                  400),
    "conditional_no_tool":    (build_conditional_no_tool,    1500),  # +300 vs v2
    "conditional_with_tool":  (build_conditional_with_tool,  1500),  # +300 vs v2
    ...
    "indirect_lumi_voice":    (build_indirect_lumi_voice,     600),  # NEW
    "shipped_tool_emphasis":  (build_shipped_tool_emphasis,   400),  # NEW
}
...
def _audit_antipatterns(records: list[dict]) -> list[str]:
    """Return list of violations: any completion containing an anti-pattern phrase."""
    violations: list[str] = []
    for i, rec in enumerate(records):
        for msg in rec["messages"]:
            if msg["role"] != "assistant":
                continue
            content = msg["content"]
            for phrase in _ANTI_PATTERNS:
                if phrase.lower() in content.lower():
                    violations.append(
                        f"Record {i} (cat={rec['category']}): "
                        f"found '{phrase}' in completion: {content[:120]!r}"
                    )
    return violations
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the Lumi synthetic fine-tuning dataset (persona v2.1).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Anti-pattern sanity check — must be zero violations before writing.
    violations = _audit_antipatterns(records)
    if violations:
        logger.error("ANTI-PATTERN VIOLATIONS FOUND — dataset NOT written:")
        for v in violations:
            logger.error("  %s", v)
        return 1
