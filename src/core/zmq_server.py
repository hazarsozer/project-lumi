"""
ZMQServer: event translation bridge between Lumi's Python Brain and the
Godot 4 Body.

This module is NOT a ZeroMQ server despite its name — it sits on top of
``IPCTransport`` (a raw TCP length-prefixed server) and provides the
event-semantic layer: it translates outbound internal events into JSON wire
frames and translates inbound JSON frames into internal event dataclasses
posted to the orchestrator's queue.
...
class ZMQServer:
    ...
    Args:
        ...
        state_machine: The shared state machine; ZMQServer registers itself
    ...
    def __init__(
        ...
        logger.debug(
            "ZMQServer initialised — transport %s:%d", host, config.port
        )
    ...
    def _handle_user_text(self, payload: dict[str, Any]) -> None:
        ...
        if not isinstance(text, str) or not text:
            logger.warning(
                "ZMQServer: user_text payload missing or empty 'text' field; "
                "dropping. payload=%r",
                payload,
            )
            return
        ...
        logger.debug("ZMQServer: posted UserTextEvent(text=%r) to queue.", text)
    ...
    def _decode(raw: bytes) -> ZMQMessage | None:
        ...
        try:
            ...
            logger.warning("ZMQServer: inbound frame is not valid UTF-8: %s", exc)
            return None

        try:
            data = json.loads(text)
        except json.JSONDecodeError as exc:
            logger.warning("ZMQServer: inbound frame is not valid JSON: %s", exc)
            return None

        if not isinstance(data, dict):
            logger.warning(
                "ZMQServer: inbound message is not a JSON object (got %s); dropping.",
                type(data).__name__,
            )
            return None
        ...
        if not isinstance(event, str):
            logger.warning(
                "ZMQServer: inbound message missing or invalid 'event' field; dropping."
            )
            return None

        if not isinstance(payload, dict):
            logger.warning(
                "ZMQServer: inbound message missing or invalid 'payload' field; dropping."
            )
            return None

        if not isinstance(timestamp, (int, float)):
            logger.warning(
                "ZMQServer: inbound message missing or invalid 'timestamp' field; dropping."
            )
            return None

        if not isinstance(version, str):
            logger.warning(
                "ZMQServer: inbound message missing or invalid 'version' field; dropping."
            )
            return None
